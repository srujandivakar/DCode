...
const createProblem = asyncHandler(async (req, res) => {
  console.log("Received problem creation request:", JSON.stringify(req.body, null, 2));
    ...
    try {
      ...
      for (let i = 0; i < results.length; i++) {
        ...
        if (result.status.id !== 3) {
          logger.error(
            `Submission ${i + 1} for ${language} failed: ${result.status.description}`
          );
          throw new ApiError(`Submission ${i + 1} for ${language} failed: ${result.status.description}`, 400);
        }
      }
    } catch (error: any) {
      logger.error(`Judge0 validation failed for ${language}: ${error.message}`);
      throw new ApiError(`Code validation failed for ${language}: ${error.message}`, 400);
    }
  }

  // Create the problem only after all validations pass
